...
var stormpathClient = new stormpath.Client({ apiKey: apiKey })
...
router.use('/:accountID/:appID/:providerID', function(req, res, next) {
	...
	stormpathClient.getAccount(accountHref, { expand: 'customData' }, function(err, account) {
		var configExists = err == null && account.customData.apps && account.customData.apps[appID] && account.customData.apps[appID][providerID]
